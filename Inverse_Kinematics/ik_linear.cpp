...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        ROS_INFO("< Planning to waypoint %d >", i, next_i);
        ...
        if (found_ik)
        {
            ...
            for (std::size_t j = 0; j < target_joints.size(); ++j)
            {
                ROS_INFO("\t(IK solution of way%d) Joint %d: %f", next_i, j, target_joints[j]);
            }
            // Jacobian도 출력
        ...
        // waypoints[i]와 waypoints[next_i] 사이에 9개의 점 추가
        std::vector<geometry_msgs::Pose> waypoints_lin;
        linear_interpolation(waypoints_lin, waypoints[i], waypoints[next_i], 10);
    
        // Motion Planning
        // Trajectory 변수: RobotTrajectory 타입으로 계산된 cartesian path를 추종하기 위한 joint trajectory가 들어있음
        // Fraction 변수: computeCartesinPath() 함수가 start부터 goal까지의 경로에 대해 몇 %의 해를 찾았냐가 들어있음
        moveit_msgs::RobotTrajectory trajectory;
        double fraction = -1.0;  // -1.0 is error

        // fraction 값을 이용해 computeCartesianPath() 함수가 경로를 조금이라도 풀 때까지 최대 10번 탐색 시도
        ...
        ROS_INFO("Cartesitan path (%.2f%% acheived)", fraction * 100.0);

        // fraction이 95% 이상일 때만 planning에 성공한 것으로 간주
        if (fraction > 0.95)
        {
            // Path visualization
